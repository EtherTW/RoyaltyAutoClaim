pragma circom 2.1.5;

include "@zk-email/zk-regex-circom/circuits/regex_helpers.circom";

// regex: ID: 0x[a-fA-F0-9]{64}\r\n
template body_idRegex(msg_bytes) {
	signal input msg[msg_bytes];
	signal output out;

	var num_bytes = msg_bytes+1;
	signal in[num_bytes];
	signal in_range_checks[msg_bytes];
	in[0]<==255;
	for (var i = 0; i < msg_bytes; i++) {
		in_range_checks[i] <== LessThan(8)([msg[i], 255]);
		in_range_checks[i] === 1;
		in[i+1] <== msg[i];
	}

	component eq[28][num_bytes];
	component and[72][num_bytes];
	component multi_or[1][num_bytes];
	signal states[num_bytes+1][73];
	signal states_tmp[num_bytes+1][73];
	signal from_zero_enabled[num_bytes+1];
	from_zero_enabled[num_bytes] <== 0;
	component state_changed[num_bytes];

	for (var i = 1; i < 73; i++) {
		states[0][i] <== 0;
	}

	for (var i = 0; i < num_bytes; i++) {
		state_changed[i] = MultiOR(72);
		states[i][0] <== 1;
		eq[0][i] = IsEqual();
		eq[0][i].in[0] <== in[i];
		eq[0][i].in[1] <== 73;
		and[0][i] = AND();
		and[0][i].a <== states[i][0];
		and[0][i].b <== eq[0][i].out;
		states_tmp[i+1][1] <== 0;
		eq[1][i] = IsEqual();
		eq[1][i].in[0] <== in[i];
		eq[1][i].in[1] <== 68;
		and[1][i] = AND();
		and[1][i].a <== states[i][1];
		and[1][i].b <== eq[1][i].out;
		states[i+1][2] <== and[1][i].out;
		eq[2][i] = IsEqual();
		eq[2][i].in[0] <== in[i];
		eq[2][i].in[1] <== 58;
		and[2][i] = AND();
		and[2][i].a <== states[i][2];
		and[2][i].b <== eq[2][i].out;
		states[i+1][3] <== and[2][i].out;
		eq[3][i] = IsEqual();
		eq[3][i].in[0] <== in[i];
		eq[3][i].in[1] <== 32;
		and[3][i] = AND();
		and[3][i].a <== states[i][3];
		and[3][i].b <== eq[3][i].out;
		states[i+1][4] <== and[3][i].out;
		eq[4][i] = IsEqual();
		eq[4][i].in[0] <== in[i];
		eq[4][i].in[1] <== 48;
		and[4][i] = AND();
		and[4][i].a <== states[i][4];
		and[4][i].b <== eq[4][i].out;
		states[i+1][5] <== and[4][i].out;
		eq[5][i] = IsEqual();
		eq[5][i].in[0] <== in[i];
		eq[5][i].in[1] <== 120;
		and[5][i] = AND();
		and[5][i].a <== states[i][5];
		and[5][i].b <== eq[5][i].out;
		states[i+1][6] <== and[5][i].out;
		eq[6][i] = IsEqual();
		eq[6][i].in[0] <== in[i];
		eq[6][i].in[1] <== 49;
		eq[7][i] = IsEqual();
		eq[7][i].in[0] <== in[i];
		eq[7][i].in[1] <== 50;
		eq[8][i] = IsEqual();
		eq[8][i].in[0] <== in[i];
		eq[8][i].in[1] <== 51;
		eq[9][i] = IsEqual();
		eq[9][i].in[0] <== in[i];
		eq[9][i].in[1] <== 52;
		eq[10][i] = IsEqual();
		eq[10][i].in[0] <== in[i];
		eq[10][i].in[1] <== 53;
		eq[11][i] = IsEqual();
		eq[11][i].in[0] <== in[i];
		eq[11][i].in[1] <== 54;
		eq[12][i] = IsEqual();
		eq[12][i].in[0] <== in[i];
		eq[12][i].in[1] <== 55;
		eq[13][i] = IsEqual();
		eq[13][i].in[0] <== in[i];
		eq[13][i].in[1] <== 56;
		eq[14][i] = IsEqual();
		eq[14][i].in[0] <== in[i];
		eq[14][i].in[1] <== 57;
		eq[15][i] = IsEqual();
		eq[15][i].in[0] <== in[i];
		eq[15][i].in[1] <== 65;
		eq[16][i] = IsEqual();
		eq[16][i].in[0] <== in[i];
		eq[16][i].in[1] <== 66;
		eq[17][i] = IsEqual();
		eq[17][i].in[0] <== in[i];
		eq[17][i].in[1] <== 67;
		eq[18][i] = IsEqual();
		eq[18][i].in[0] <== in[i];
		eq[18][i].in[1] <== 69;
		eq[19][i] = IsEqual();
		eq[19][i].in[0] <== in[i];
		eq[19][i].in[1] <== 70;
		eq[20][i] = IsEqual();
		eq[20][i].in[0] <== in[i];
		eq[20][i].in[1] <== 97;
		eq[21][i] = IsEqual();
		eq[21][i].in[0] <== in[i];
		eq[21][i].in[1] <== 98;
		eq[22][i] = IsEqual();
		eq[22][i].in[0] <== in[i];
		eq[22][i].in[1] <== 99;
		eq[23][i] = IsEqual();
		eq[23][i].in[0] <== in[i];
		eq[23][i].in[1] <== 100;
		eq[24][i] = IsEqual();
		eq[24][i].in[0] <== in[i];
		eq[24][i].in[1] <== 101;
		eq[25][i] = IsEqual();
		eq[25][i].in[0] <== in[i];
		eq[25][i].in[1] <== 102;
		and[6][i] = AND();
		and[6][i].a <== states[i][6];
		multi_or[0][i] = MultiOR(22);
		multi_or[0][i].in[0] <== eq[4][i].out;
		multi_or[0][i].in[1] <== eq[6][i].out;
		multi_or[0][i].in[2] <== eq[7][i].out;
		multi_or[0][i].in[3] <== eq[8][i].out;
		multi_or[0][i].in[4] <== eq[9][i].out;
		multi_or[0][i].in[5] <== eq[10][i].out;
		multi_or[0][i].in[6] <== eq[11][i].out;
		multi_or[0][i].in[7] <== eq[12][i].out;
		multi_or[0][i].in[8] <== eq[13][i].out;
		multi_or[0][i].in[9] <== eq[14][i].out;
		multi_or[0][i].in[10] <== eq[15][i].out;
		multi_or[0][i].in[11] <== eq[16][i].out;
		multi_or[0][i].in[12] <== eq[17][i].out;
		multi_or[0][i].in[13] <== eq[1][i].out;
		multi_or[0][i].in[14] <== eq[18][i].out;
		multi_or[0][i].in[15] <== eq[19][i].out;
		multi_or[0][i].in[16] <== eq[20][i].out;
		multi_or[0][i].in[17] <== eq[21][i].out;
		multi_or[0][i].in[18] <== eq[22][i].out;
		multi_or[0][i].in[19] <== eq[23][i].out;
		multi_or[0][i].in[20] <== eq[24][i].out;
		multi_or[0][i].in[21] <== eq[25][i].out;
		and[6][i].b <== multi_or[0][i].out;
		states[i+1][7] <== and[6][i].out;
		and[7][i] = AND();
		and[7][i].a <== states[i][7];
		and[7][i].b <== multi_or[0][i].out;
		states[i+1][8] <== and[7][i].out;
		and[8][i] = AND();
		and[8][i].a <== states[i][8];
		and[8][i].b <== multi_or[0][i].out;
		states[i+1][9] <== and[8][i].out;
		and[9][i] = AND();
		and[9][i].a <== states[i][9];
		and[9][i].b <== multi_or[0][i].out;
		states[i+1][10] <== and[9][i].out;
		and[10][i] = AND();
		and[10][i].a <== states[i][10];
		and[10][i].b <== multi_or[0][i].out;
		states[i+1][11] <== and[10][i].out;
		and[11][i] = AND();
		and[11][i].a <== states[i][11];
		and[11][i].b <== multi_or[0][i].out;
		states[i+1][12] <== and[11][i].out;
		and[12][i] = AND();
		and[12][i].a <== states[i][12];
		and[12][i].b <== multi_or[0][i].out;
		states[i+1][13] <== and[12][i].out;
		and[13][i] = AND();
		and[13][i].a <== states[i][13];
		and[13][i].b <== multi_or[0][i].out;
		states[i+1][14] <== and[13][i].out;
		and[14][i] = AND();
		and[14][i].a <== states[i][14];
		and[14][i].b <== multi_or[0][i].out;
		states[i+1][15] <== and[14][i].out;
		and[15][i] = AND();
		and[15][i].a <== states[i][15];
		and[15][i].b <== multi_or[0][i].out;
		states[i+1][16] <== and[15][i].out;
		and[16][i] = AND();
		and[16][i].a <== states[i][16];
		and[16][i].b <== multi_or[0][i].out;
		states[i+1][17] <== and[16][i].out;
		and[17][i] = AND();
		and[17][i].a <== states[i][17];
		and[17][i].b <== multi_or[0][i].out;
		states[i+1][18] <== and[17][i].out;
		and[18][i] = AND();
		and[18][i].a <== states[i][18];
		and[18][i].b <== multi_or[0][i].out;
		states[i+1][19] <== and[18][i].out;
		and[19][i] = AND();
		and[19][i].a <== states[i][19];
		and[19][i].b <== multi_or[0][i].out;
		states[i+1][20] <== and[19][i].out;
		and[20][i] = AND();
		and[20][i].a <== states[i][20];
		and[20][i].b <== multi_or[0][i].out;
		states[i+1][21] <== and[20][i].out;
		and[21][i] = AND();
		and[21][i].a <== states[i][21];
		and[21][i].b <== multi_or[0][i].out;
		states[i+1][22] <== and[21][i].out;
		and[22][i] = AND();
		and[22][i].a <== states[i][22];
		and[22][i].b <== multi_or[0][i].out;
		states[i+1][23] <== and[22][i].out;
		and[23][i] = AND();
		and[23][i].a <== states[i][23];
		and[23][i].b <== multi_or[0][i].out;
		states[i+1][24] <== and[23][i].out;
		and[24][i] = AND();
		and[24][i].a <== states[i][24];
		and[24][i].b <== multi_or[0][i].out;
		states[i+1][25] <== and[24][i].out;
		and[25][i] = AND();
		and[25][i].a <== states[i][25];
		and[25][i].b <== multi_or[0][i].out;
		states[i+1][26] <== and[25][i].out;
		and[26][i] = AND();
		and[26][i].a <== states[i][26];
		and[26][i].b <== multi_or[0][i].out;
		states[i+1][27] <== and[26][i].out;
		and[27][i] = AND();
		and[27][i].a <== states[i][27];
		and[27][i].b <== multi_or[0][i].out;
		states[i+1][28] <== and[27][i].out;
		and[28][i] = AND();
		and[28][i].a <== states[i][28];
		and[28][i].b <== multi_or[0][i].out;
		states[i+1][29] <== and[28][i].out;
		and[29][i] = AND();
		and[29][i].a <== states[i][29];
		and[29][i].b <== multi_or[0][i].out;
		states[i+1][30] <== and[29][i].out;
		and[30][i] = AND();
		and[30][i].a <== states[i][30];
		and[30][i].b <== multi_or[0][i].out;
		states[i+1][31] <== and[30][i].out;
		and[31][i] = AND();
		and[31][i].a <== states[i][31];
		and[31][i].b <== multi_or[0][i].out;
		states[i+1][32] <== and[31][i].out;
		and[32][i] = AND();
		and[32][i].a <== states[i][32];
		and[32][i].b <== multi_or[0][i].out;
		states[i+1][33] <== and[32][i].out;
		and[33][i] = AND();
		and[33][i].a <== states[i][33];
		and[33][i].b <== multi_or[0][i].out;
		states[i+1][34] <== and[33][i].out;
		and[34][i] = AND();
		and[34][i].a <== states[i][34];
		and[34][i].b <== multi_or[0][i].out;
		states[i+1][35] <== and[34][i].out;
		and[35][i] = AND();
		and[35][i].a <== states[i][35];
		and[35][i].b <== multi_or[0][i].out;
		states[i+1][36] <== and[35][i].out;
		and[36][i] = AND();
		and[36][i].a <== states[i][36];
		and[36][i].b <== multi_or[0][i].out;
		states[i+1][37] <== and[36][i].out;
		and[37][i] = AND();
		and[37][i].a <== states[i][37];
		and[37][i].b <== multi_or[0][i].out;
		states[i+1][38] <== and[37][i].out;
		and[38][i] = AND();
		and[38][i].a <== states[i][38];
		and[38][i].b <== multi_or[0][i].out;
		states[i+1][39] <== and[38][i].out;
		and[39][i] = AND();
		and[39][i].a <== states[i][39];
		and[39][i].b <== multi_or[0][i].out;
		states[i+1][40] <== and[39][i].out;
		and[40][i] = AND();
		and[40][i].a <== states[i][40];
		and[40][i].b <== multi_or[0][i].out;
		states[i+1][41] <== and[40][i].out;
		and[41][i] = AND();
		and[41][i].a <== states[i][41];
		and[41][i].b <== multi_or[0][i].out;
		states[i+1][42] <== and[41][i].out;
		and[42][i] = AND();
		and[42][i].a <== states[i][42];
		and[42][i].b <== multi_or[0][i].out;
		states[i+1][43] <== and[42][i].out;
		and[43][i] = AND();
		and[43][i].a <== states[i][43];
		and[43][i].b <== multi_or[0][i].out;
		states[i+1][44] <== and[43][i].out;
		and[44][i] = AND();
		and[44][i].a <== states[i][44];
		and[44][i].b <== multi_or[0][i].out;
		states[i+1][45] <== and[44][i].out;
		and[45][i] = AND();
		and[45][i].a <== states[i][45];
		and[45][i].b <== multi_or[0][i].out;
		states[i+1][46] <== and[45][i].out;
		and[46][i] = AND();
		and[46][i].a <== states[i][46];
		and[46][i].b <== multi_or[0][i].out;
		states[i+1][47] <== and[46][i].out;
		and[47][i] = AND();
		and[47][i].a <== states[i][47];
		and[47][i].b <== multi_or[0][i].out;
		states[i+1][48] <== and[47][i].out;
		and[48][i] = AND();
		and[48][i].a <== states[i][48];
		and[48][i].b <== multi_or[0][i].out;
		states[i+1][49] <== and[48][i].out;
		and[49][i] = AND();
		and[49][i].a <== states[i][49];
		and[49][i].b <== multi_or[0][i].out;
		states[i+1][50] <== and[49][i].out;
		and[50][i] = AND();
		and[50][i].a <== states[i][50];
		and[50][i].b <== multi_or[0][i].out;
		states[i+1][51] <== and[50][i].out;
		and[51][i] = AND();
		and[51][i].a <== states[i][51];
		and[51][i].b <== multi_or[0][i].out;
		states[i+1][52] <== and[51][i].out;
		and[52][i] = AND();
		and[52][i].a <== states[i][52];
		and[52][i].b <== multi_or[0][i].out;
		states[i+1][53] <== and[52][i].out;
		and[53][i] = AND();
		and[53][i].a <== states[i][53];
		and[53][i].b <== multi_or[0][i].out;
		states[i+1][54] <== and[53][i].out;
		and[54][i] = AND();
		and[54][i].a <== states[i][54];
		and[54][i].b <== multi_or[0][i].out;
		states[i+1][55] <== and[54][i].out;
		and[55][i] = AND();
		and[55][i].a <== states[i][55];
		and[55][i].b <== multi_or[0][i].out;
		states[i+1][56] <== and[55][i].out;
		and[56][i] = AND();
		and[56][i].a <== states[i][56];
		and[56][i].b <== multi_or[0][i].out;
		states[i+1][57] <== and[56][i].out;
		and[57][i] = AND();
		and[57][i].a <== states[i][57];
		and[57][i].b <== multi_or[0][i].out;
		states[i+1][58] <== and[57][i].out;
		and[58][i] = AND();
		and[58][i].a <== states[i][58];
		and[58][i].b <== multi_or[0][i].out;
		states[i+1][59] <== and[58][i].out;
		and[59][i] = AND();
		and[59][i].a <== states[i][59];
		and[59][i].b <== multi_or[0][i].out;
		states[i+1][60] <== and[59][i].out;
		and[60][i] = AND();
		and[60][i].a <== states[i][60];
		and[60][i].b <== multi_or[0][i].out;
		states[i+1][61] <== and[60][i].out;
		and[61][i] = AND();
		and[61][i].a <== states[i][61];
		and[61][i].b <== multi_or[0][i].out;
		states[i+1][62] <== and[61][i].out;
		and[62][i] = AND();
		and[62][i].a <== states[i][62];
		and[62][i].b <== multi_or[0][i].out;
		states[i+1][63] <== and[62][i].out;
		and[63][i] = AND();
		and[63][i].a <== states[i][63];
		and[63][i].b <== multi_or[0][i].out;
		states[i+1][64] <== and[63][i].out;
		and[64][i] = AND();
		and[64][i].a <== states[i][64];
		and[64][i].b <== multi_or[0][i].out;
		states[i+1][65] <== and[64][i].out;
		and[65][i] = AND();
		and[65][i].a <== states[i][65];
		and[65][i].b <== multi_or[0][i].out;
		states[i+1][66] <== and[65][i].out;
		and[66][i] = AND();
		and[66][i].a <== states[i][66];
		and[66][i].b <== multi_or[0][i].out;
		states[i+1][67] <== and[66][i].out;
		and[67][i] = AND();
		and[67][i].a <== states[i][67];
		and[67][i].b <== multi_or[0][i].out;
		states[i+1][68] <== and[67][i].out;
		and[68][i] = AND();
		and[68][i].a <== states[i][68];
		and[68][i].b <== multi_or[0][i].out;
		states[i+1][69] <== and[68][i].out;
		and[69][i] = AND();
		and[69][i].a <== states[i][69];
		and[69][i].b <== multi_or[0][i].out;
		states[i+1][70] <== and[69][i].out;
		eq[26][i] = IsEqual();
		eq[26][i].in[0] <== in[i];
		eq[26][i].in[1] <== 13;
		and[70][i] = AND();
		and[70][i].a <== states[i][70];
		and[70][i].b <== eq[26][i].out;
		states[i+1][71] <== and[70][i].out;
		eq[27][i] = IsEqual();
		eq[27][i].in[0] <== in[i];
		eq[27][i].in[1] <== 10;
		and[71][i] = AND();
		and[71][i].a <== states[i][71];
		and[71][i].b <== eq[27][i].out;
		states[i+1][72] <== and[71][i].out;
		from_zero_enabled[i] <== MultiNOR(72)([states_tmp[i+1][1], states[i+1][2], states[i+1][3], states[i+1][4], states[i+1][5], states[i+1][6], states[i+1][7], states[i+1][8], states[i+1][9], states[i+1][10], states[i+1][11], states[i+1][12], states[i+1][13], states[i+1][14], states[i+1][15], states[i+1][16], states[i+1][17], states[i+1][18], states[i+1][19], states[i+1][20], states[i+1][21], states[i+1][22], states[i+1][23], states[i+1][24], states[i+1][25], states[i+1][26], states[i+1][27], states[i+1][28], states[i+1][29], states[i+1][30], states[i+1][31], states[i+1][32], states[i+1][33], states[i+1][34], states[i+1][35], states[i+1][36], states[i+1][37], states[i+1][38], states[i+1][39], states[i+1][40], states[i+1][41], states[i+1][42], states[i+1][43], states[i+1][44], states[i+1][45], states[i+1][46], states[i+1][47], states[i+1][48], states[i+1][49], states[i+1][50], states[i+1][51], states[i+1][52], states[i+1][53], states[i+1][54], states[i+1][55], states[i+1][56], states[i+1][57], states[i+1][58], states[i+1][59], states[i+1][60], states[i+1][61], states[i+1][62], states[i+1][63], states[i+1][64], states[i+1][65], states[i+1][66], states[i+1][67], states[i+1][68], states[i+1][69], states[i+1][70], states[i+1][71], states[i+1][72]]);
		states[i+1][1] <== MultiOR(2)([states_tmp[i+1][1], from_zero_enabled[i] * and[0][i].out]);
		state_changed[i].in[0] <== states[i+1][1];
		state_changed[i].in[1] <== states[i+1][2];
		state_changed[i].in[2] <== states[i+1][3];
		state_changed[i].in[3] <== states[i+1][4];
		state_changed[i].in[4] <== states[i+1][5];
		state_changed[i].in[5] <== states[i+1][6];
		state_changed[i].in[6] <== states[i+1][7];
		state_changed[i].in[7] <== states[i+1][8];
		state_changed[i].in[8] <== states[i+1][9];
		state_changed[i].in[9] <== states[i+1][10];
		state_changed[i].in[10] <== states[i+1][11];
		state_changed[i].in[11] <== states[i+1][12];
		state_changed[i].in[12] <== states[i+1][13];
		state_changed[i].in[13] <== states[i+1][14];
		state_changed[i].in[14] <== states[i+1][15];
		state_changed[i].in[15] <== states[i+1][16];
		state_changed[i].in[16] <== states[i+1][17];
		state_changed[i].in[17] <== states[i+1][18];
		state_changed[i].in[18] <== states[i+1][19];
		state_changed[i].in[19] <== states[i+1][20];
		state_changed[i].in[20] <== states[i+1][21];
		state_changed[i].in[21] <== states[i+1][22];
		state_changed[i].in[22] <== states[i+1][23];
		state_changed[i].in[23] <== states[i+1][24];
		state_changed[i].in[24] <== states[i+1][25];
		state_changed[i].in[25] <== states[i+1][26];
		state_changed[i].in[26] <== states[i+1][27];
		state_changed[i].in[27] <== states[i+1][28];
		state_changed[i].in[28] <== states[i+1][29];
		state_changed[i].in[29] <== states[i+1][30];
		state_changed[i].in[30] <== states[i+1][31];
		state_changed[i].in[31] <== states[i+1][32];
		state_changed[i].in[32] <== states[i+1][33];
		state_changed[i].in[33] <== states[i+1][34];
		state_changed[i].in[34] <== states[i+1][35];
		state_changed[i].in[35] <== states[i+1][36];
		state_changed[i].in[36] <== states[i+1][37];
		state_changed[i].in[37] <== states[i+1][38];
		state_changed[i].in[38] <== states[i+1][39];
		state_changed[i].in[39] <== states[i+1][40];
		state_changed[i].in[40] <== states[i+1][41];
		state_changed[i].in[41] <== states[i+1][42];
		state_changed[i].in[42] <== states[i+1][43];
		state_changed[i].in[43] <== states[i+1][44];
		state_changed[i].in[44] <== states[i+1][45];
		state_changed[i].in[45] <== states[i+1][46];
		state_changed[i].in[46] <== states[i+1][47];
		state_changed[i].in[47] <== states[i+1][48];
		state_changed[i].in[48] <== states[i+1][49];
		state_changed[i].in[49] <== states[i+1][50];
		state_changed[i].in[50] <== states[i+1][51];
		state_changed[i].in[51] <== states[i+1][52];
		state_changed[i].in[52] <== states[i+1][53];
		state_changed[i].in[53] <== states[i+1][54];
		state_changed[i].in[54] <== states[i+1][55];
		state_changed[i].in[55] <== states[i+1][56];
		state_changed[i].in[56] <== states[i+1][57];
		state_changed[i].in[57] <== states[i+1][58];
		state_changed[i].in[58] <== states[i+1][59];
		state_changed[i].in[59] <== states[i+1][60];
		state_changed[i].in[60] <== states[i+1][61];
		state_changed[i].in[61] <== states[i+1][62];
		state_changed[i].in[62] <== states[i+1][63];
		state_changed[i].in[63] <== states[i+1][64];
		state_changed[i].in[64] <== states[i+1][65];
		state_changed[i].in[65] <== states[i+1][66];
		state_changed[i].in[66] <== states[i+1][67];
		state_changed[i].in[67] <== states[i+1][68];
		state_changed[i].in[68] <== states[i+1][69];
		state_changed[i].in[69] <== states[i+1][70];
		state_changed[i].in[70] <== states[i+1][71];
		state_changed[i].in[71] <== states[i+1][72];
	}

	component is_accepted = MultiOR(num_bytes+1);
	for (var i = 0; i <= num_bytes; i++) {
		is_accepted.in[i] <== states[i][72];
	}
	out <== is_accepted.out;
	signal is_consecutive[msg_bytes+1][3];
	is_consecutive[msg_bytes][2] <== 0;
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][72] * (1 - is_consecutive[msg_bytes-i][2]) + is_consecutive[msg_bytes-i][2];
		is_consecutive[msg_bytes-1-i][1] <== state_changed[msg_bytes-i].out * is_consecutive[msg_bytes-1-i][0];
		is_consecutive[msg_bytes-1-i][2] <== ORAnd()([(1 - from_zero_enabled[msg_bytes-i+1]), states[num_bytes-i][72], is_consecutive[msg_bytes-1-i][1]]);
	}
	// substrings calculated: [{(4, 5), (5, 6), (6, 7), (7, 8), (8, 9), (9, 10), (10, 11), (11, 12), (12, 13), (13, 14), (14, 15), (15, 16), (16, 17), (17, 18), (18, 19), (19, 20), (20, 21), (21, 22), (22, 23), (23, 24), (24, 25), (25, 26), (26, 27), (27, 28), (28, 29), (29, 30), (30, 31), (31, 32), (32, 33), (33, 34), (34, 35), (35, 36), (36, 37), (37, 38), (38, 39), (39, 40), (40, 41), (41, 42), (42, 43), (43, 44), (44, 45), (45, 46), (46, 47), (47, 48), (48, 49), (49, 50), (50, 51), (51, 52), (52, 53), (53, 54), (54, 55), (55, 56), (56, 57), (57, 58), (58, 59), (59, 60), (60, 61), (61, 62), (62, 63), (63, 64), (64, 65), (65, 66), (66, 67), (67, 68), (68, 69), (69, 70)}]
	signal prev_states0[66][msg_bytes];
	signal is_substr0[msg_bytes];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		 // the 0-th substring transitions: [(4, 5), (5, 6), (6, 7), (7, 8), (8, 9), (9, 10), (10, 11), (11, 12), (12, 13), (13, 14), (14, 15), (15, 16), (16, 17), (17, 18), (18, 19), (19, 20), (20, 21), (21, 22), (22, 23), (23, 24), (24, 25), (25, 26), (26, 27), (27, 28), (28, 29), (29, 30), (30, 31), (31, 32), (32, 33), (33, 34), (34, 35), (35, 36), (36, 37), (37, 38), (38, 39), (39, 40), (40, 41), (41, 42), (42, 43), (43, 44), (44, 45), (45, 46), (46, 47), (47, 48), (48, 49), (49, 50), (50, 51), (51, 52), (52, 53), (53, 54), (54, 55), (55, 56), (56, 57), (57, 58), (58, 59), (59, 60), (60, 61), (61, 62), (62, 63), (63, 64), (64, 65), (65, 66), (66, 67), (67, 68), (68, 69), (69, 70)]
		prev_states0[0][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][4];
		prev_states0[1][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][5];
		prev_states0[2][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][6];
		prev_states0[3][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][7];
		prev_states0[4][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][8];
		prev_states0[5][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][9];
		prev_states0[6][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][10];
		prev_states0[7][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][11];
		prev_states0[8][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][12];
		prev_states0[9][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][13];
		prev_states0[10][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][14];
		prev_states0[11][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][15];
		prev_states0[12][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][16];
		prev_states0[13][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][17];
		prev_states0[14][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][18];
		prev_states0[15][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][19];
		prev_states0[16][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][20];
		prev_states0[17][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][21];
		prev_states0[18][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][22];
		prev_states0[19][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][23];
		prev_states0[20][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][24];
		prev_states0[21][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][25];
		prev_states0[22][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][26];
		prev_states0[23][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][27];
		prev_states0[24][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][28];
		prev_states0[25][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][29];
		prev_states0[26][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][30];
		prev_states0[27][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][31];
		prev_states0[28][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][32];
		prev_states0[29][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][33];
		prev_states0[30][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][34];
		prev_states0[31][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][35];
		prev_states0[32][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][36];
		prev_states0[33][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][37];
		prev_states0[34][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][38];
		prev_states0[35][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][39];
		prev_states0[36][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][40];
		prev_states0[37][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][41];
		prev_states0[38][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][42];
		prev_states0[39][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][43];
		prev_states0[40][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][44];
		prev_states0[41][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][45];
		prev_states0[42][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][46];
		prev_states0[43][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][47];
		prev_states0[44][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][48];
		prev_states0[45][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][49];
		prev_states0[46][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][50];
		prev_states0[47][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][51];
		prev_states0[48][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][52];
		prev_states0[49][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][53];
		prev_states0[50][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][54];
		prev_states0[51][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][55];
		prev_states0[52][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][56];
		prev_states0[53][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][57];
		prev_states0[54][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][58];
		prev_states0[55][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][59];
		prev_states0[56][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][60];
		prev_states0[57][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][61];
		prev_states0[58][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][62];
		prev_states0[59][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][63];
		prev_states0[60][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][64];
		prev_states0[61][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][65];
		prev_states0[62][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][66];
		prev_states0[63][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][67];
		prev_states0[64][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][68];
		prev_states0[65][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][69];
		is_substr0[i] <== MultiOR(66)([prev_states0[0][i] * states[i+2][5], prev_states0[1][i] * states[i+2][6], prev_states0[2][i] * states[i+2][7], prev_states0[3][i] * states[i+2][8], prev_states0[4][i] * states[i+2][9], prev_states0[5][i] * states[i+2][10], prev_states0[6][i] * states[i+2][11], prev_states0[7][i] * states[i+2][12], prev_states0[8][i] * states[i+2][13], prev_states0[9][i] * states[i+2][14], prev_states0[10][i] * states[i+2][15], prev_states0[11][i] * states[i+2][16], prev_states0[12][i] * states[i+2][17], prev_states0[13][i] * states[i+2][18], prev_states0[14][i] * states[i+2][19], prev_states0[15][i] * states[i+2][20], prev_states0[16][i] * states[i+2][21], prev_states0[17][i] * states[i+2][22], prev_states0[18][i] * states[i+2][23], prev_states0[19][i] * states[i+2][24], prev_states0[20][i] * states[i+2][25], prev_states0[21][i] * states[i+2][26], prev_states0[22][i] * states[i+2][27], prev_states0[23][i] * states[i+2][28], prev_states0[24][i] * states[i+2][29], prev_states0[25][i] * states[i+2][30], prev_states0[26][i] * states[i+2][31], prev_states0[27][i] * states[i+2][32], prev_states0[28][i] * states[i+2][33], prev_states0[29][i] * states[i+2][34], prev_states0[30][i] * states[i+2][35], prev_states0[31][i] * states[i+2][36], prev_states0[32][i] * states[i+2][37], prev_states0[33][i] * states[i+2][38], prev_states0[34][i] * states[i+2][39], prev_states0[35][i] * states[i+2][40], prev_states0[36][i] * states[i+2][41], prev_states0[37][i] * states[i+2][42], prev_states0[38][i] * states[i+2][43], prev_states0[39][i] * states[i+2][44], prev_states0[40][i] * states[i+2][45], prev_states0[41][i] * states[i+2][46], prev_states0[42][i] * states[i+2][47], prev_states0[43][i] * states[i+2][48], prev_states0[44][i] * states[i+2][49], prev_states0[45][i] * states[i+2][50], prev_states0[46][i] * states[i+2][51], prev_states0[47][i] * states[i+2][52], prev_states0[48][i] * states[i+2][53], prev_states0[49][i] * states[i+2][54], prev_states0[50][i] * states[i+2][55], prev_states0[51][i] * states[i+2][56], prev_states0[52][i] * states[i+2][57], prev_states0[53][i] * states[i+2][58], prev_states0[54][i] * states[i+2][59], prev_states0[55][i] * states[i+2][60], prev_states0[56][i] * states[i+2][61], prev_states0[57][i] * states[i+2][62], prev_states0[58][i] * states[i+2][63], prev_states0[59][i] * states[i+2][64], prev_states0[60][i] * states[i+2][65], prev_states0[61][i] * states[i+2][66], prev_states0[62][i] * states[i+2][67], prev_states0[63][i] * states[i+2][68], prev_states0[64][i] * states[i+2][69], prev_states0[65][i] * states[i+2][70]]);
		is_reveal0[i] <== MultiAND(3)([out, is_substr0[i], is_consecutive[i][2]]);
		reveal0[i] <== in[i+1] * is_reveal0[i];
	}
}