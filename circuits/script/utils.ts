...
export type CircuitInputsMainId = {
	header: BoundedVec
	pubkey: RSAPubkey
	signature: string[]
	dkim_header_seq: Sequence
...
/**
 * Write circuit inputs to Prover.toml for main_id circuit
 * @param circuitTargetPath - path to circuits/target directory
 * @param inputs - circuit inputs object for main_id
 */
export function writeProverTomlMainId(circuitTargetPath: string, inputs: CircuitInputsMainId) {
	const fs = require('fs')
	const path = require('path')
	const tomlLines: string[] = []
